...
\academicprogram{Mathematics and Computer Science}
\gradyear{2019}
\advisor{Dr.\ Nathan Fox (Mathematics and Computer Science)}
...
\begin{abstract}
	Natural language processing systems have revolutionized the way we interact with technology over the last several years, from simple calendar management with digital assitants to safer driving by allowing dictated communications behind the wheel. Though everyday activities are a bit easier, these systems have not lowered the barrier of entry to mathematical theory. While, for example, Wolfram Alpha allows users to input English statements to find mathematical values, but it does not process propositional statements. In this study, we seek to create a translation system to link propositions by users stated in everyday English with an automated proving system called Vampire. While the system is limited, it works well within certain domains.
\end{abstract}
...
\bibliography{Bibliography} % load our Bibliography file
